...
class HudDemoController: UIViewController {
    enum ProgressMode {
        case special
    ...
    func doSomeWorkWithProgress() {
        ...
        while progress < 1.0 {
            ...
            DispatchQueue.main.async {[weak self] in
                ...
                YFProgressHud.hud(for: rootView)?.progress = progress
            }
            usleep(50000)
        }
    }
    ...
    func doSomeWorkWithMixedProgress(_ hud: YFProgressHud) {
        sleep(2)
...
extension HudDemoController {
    
    func indeterminateExample() {
        guard let rootView = navigationController?.view else { return }
        
        let hud = YFProgressHud.showAdded(to: rootView, animated: true)
...
extension HudDemoController: UITableViewDataSource {
...
extension HudDemoController: UITableViewDelegate {
